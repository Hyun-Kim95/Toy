        ...
        function buildCalendar() {
            ...
            for (i=1; i <=lastDate; i++){ // 날짜 채우기
                ...
                $("#calendar tbody:last").append("<td class='date flex-shrink shadow rounded container sm:text-xl md:text-2xl'>"
                		+ "<button class='btn-primary px-3 2xl:px-24 xl:px-20 lg:px-16 md:px-10 sm:px-9 2xl:px-24 xl:px-20 lg:px-16 md:px-10 sm:px-9 sm:py-4 md:py-5 lg:py-8' name='selectedDate' value=\'" + nowYear + "-" + (nowMonth+1) + "-" + i + "\'>" 
                		+ i 
                		+ "</button>" +"</td>");
            }
...
<section class="section-1">
	<form action="../diaryList/detail" class="mx-2">
		<table id="calendar"
			class="bg-white shadow rounded container mx-auto p-8 mt-8">
			<thead>
				<tr class="shadow">
					<th><input class="bg-white sm:text-3xl md:text-5xl rounded"
						name="preMon" type="button" value="◀"></th>
					<th colspan="5" class="year_mon sm:text-3xl md:text-5xl p-5"></th>
					<th><input class="bg-white sm:text-3xl md:text-5xl rounded"
						name="nextMon" type="button" value="▶"></th>
				</tr>
				...
				<c:forEach items="${missings}" var="missing">
					<div class="flex">
						<c:if test="${missing != null}">
							<a href="../diaryList/add?selectedDate=${missing}"
								name="selectedDay"
								class="m-3 btn-primary bg-blue-500 hover:bg-blue-dark text-white font-bold py-2 px-4 rounded"
								value="${missing}">${missing}</a>
						</c:if>
					</div>
				</c:forEach>
			</div>
